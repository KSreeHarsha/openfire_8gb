...
public class ConversationManager implements Startable, ComponentEventListener {
	...
	try{
	 mongoClient = new MongoClient("172.31.10.109");
	} catch (Exception e) {
	    return;
  	}
        // get handle to "mydb"
        DB db = mongoClient.getDB("openfire");

        // get a collection object to work with
        DBCollection testCollection = db.getCollection("ofMessageArchive");


        // make a document and insert it
	//openfire.ofMessageArchive.insert({conversationID:2799,fromJID:"two@137.110.90.7",fromJIDResource:"29cf72b7",
	//toJID:"one@137.110.90.7",toJIDResource:"9df75905",sentDate:1401414799178,body:"hey"})
        BasicDBObject doc = new BasicDBObject("conversationID",ConversationID).append("fromJID",fromJID)
                                                                .append("fromJIDResource",fromResource)
                                                                .append("toJID",toJID)
                                                                .append("toJIDResource",toResource)
			...
			if (!messageQueue.isEmpty() || !conversationQueue.isEmpty() || !participantQueue.isEmpty()) {
				...
				try {
					...
					while ((message = messageQueue.poll()) != null) {
						...
						Log.error("###***###:Saving Message to Mg db");
						insert_mdb(message.getConversationID(),message.getFromJID().toBareJID(),message.getFromJID().getResource(),
						message.getToJID().toBareJID(),message.getToJID().getResource()
						,message.getSentDate().getTime(),message.getBody());
						if(false){
